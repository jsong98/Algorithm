...
class 섬지키기 {
        ...
        for (int i = 0; i <= n + 1; i++) {
            for (int j = 0; j <= n + 1; j++) {
                if (i == 0 || i == n + 1 || j == 0 || j == n + 1) {
                    q.add(new int[]{i, j}); // 초기 물의 위치를 queue에 담고 시작, bfs 최적화 기법 중 하나
                    check[i][j] = true;
                } else
                    check[i][j] = false;
            }
    ...
    public int maxArea(int M, int[] mStructure, int mSeaLevel) {
        int ret = -1;
        if (M == 1) {   // 구조물의 크기가 1인 경우, 모든 위치에 다 놓고 전부 bfs 돌림
            for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= n; j++) {
        ...
        for (Candidate wall : candidate[hash]) {    // 방향에 맞게 bfs
